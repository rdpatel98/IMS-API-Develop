                ...
                using (var context = new StoreContext())
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            var items = context.PurchaseOrderItems.Where(x => x.PurchaseOrderId == savePurchaseOrder.PurchaseOrder.PurchaseOrderId);
                            var dbIds = savePurchaseOrder.PurchaseOrderItems.Where(x => x.PurchaseOrderItemsId > 0).Select(y => y.PurchaseOrderItemsId);
                            var deleteItems = items.Where(x => !dbIds.Contains(x.PurchaseOrderItemsId));
                            context.PurchaseOrderItems.RemoveRange(deleteItems);
                            ...
                            foreach (var purchaseOrderItem in savePurchaseOrder.PurchaseOrderItems)
                            {
                                ...
                                else
                                {
                                    ...
                                    poItems.PurchaseOrderId = savePurchaseOrder.PurchaseOrder.PurchaseOrderId;
                ...
                using (var context = new StoreContext())
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            var purchaseReceive = context.PurchaseReceive.FirstOrDefault(x => x.PurchaseOrderId == savePurchaseOrder.PurchaseReceive.PurchaseOrderId);
                            if (purchaseReceive != null)
                                purchaseReceive.PurchaseReceiveStatus = 4;
                            var invoice = context.Invoice.FirstOrDefault(x => x.PurchaseOrderId == savePurchaseOrder.PurchaseReceive.PurchaseOrderId);
                            if (invoice != null)
                    ...
                    try
                    {
                        ...
                        using (var command = new SqlCommand(sqlQuery, con))
                        {
                            ...
                            using (var reader = command.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    var itemCategory = new ItemCategories();
                                    itemCategory.ItemId = int.Parse(reader["ItemId"].ToString());
                                    itemCategory.CategoryName = reader["CategoryName"].ToString();
                        ...
                        using (var command = new SqlCommand(itemsSqlQuery, con))
                        {
                            ...
                            using (var reader = command.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    ...
                                    report.ItemId = int.Parse(reader["ItemId"].ToString());
                        ...
                        response.ItemCategories = itemCategories.GroupBy(x => new { x.ItemCategoryId, x.CategoryName, x.CategoryId }).Select(x => new ItemCategoryHeader()
                        {
                            ...
                            ItemCategoryId = x.Key.ItemCategoryId
                        }).ToList();
                        con.Close();
                        return CommonUtils.CreateSuccessApiResponse(response);
